...
int insertItem(int fd,DataItem item){
   ...
   int hashIndx = hashCode(item.key); 
   int Offset = hashIndx*sizeof(Bucket);
	int count = 0; // number of records probed 
	int continueSearch = 1; // flag to continue searching for empty space
	for(int SearchOffest = Offset ; continueSearch ==1 ; SearchOffest += sizeof(DataItem)){
		
		DataItem temp ;
		ssize_t result = pread(fd,&temp,sizeof(DataItem), SearchOffest);
		if(result < 0)
		{ 	  perror("some error occurred in pread");
			  return -1;
		} else if (temp.valid == 0 ) { //empty space found or end of file
			
			pwrite(fd,&item,sizeof(DataItem), SearchOffest);
			continueSearch = 0;
		} else {
			count++;
		}
